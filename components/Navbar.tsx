        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div ref={langRef} className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-slate-200 text-slate-600 hover:border-slate-300 hover:text-slate-900 text-sm font-medium transition-colors duration-150 select-none"
              >
                ...
                >
                  {(["en", "es", "pt"] as Lang[]).map((l) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-400 font-mono">{LANG_LABELS[l]}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="md:hidden flex items-center gap-2">
            <div ref={langRef} className="relative">
              <button
                onClick={() => setLangOpen((v) => !v)}
                className="px-2.5 py-1.5 rounded-md border border-slate-200 text-slate-600 text-xs font-semibold"
              >
                {LANG_LABELS[lang]}
              </button>
              {langOpen && (
                <div className="absolute right-0 top-full mt-1 w-32 rounded-lg border border-slate-200 bg-white shadow-lg py-1 z-50">
                  {(["en", "es", "pt"] as Lang[]).map((l) => (
                    <button
                      key={l}
                      onClick={() => handleLang(l)}
