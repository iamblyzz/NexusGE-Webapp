    ...
    return (
      <section id="intake-form" className="py-24 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 px-8 py-14 text-center">
    ...
    <section id="intake-form" className="py-24 bg-white">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-slate-500 text-base">{t.subheadline}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2.5 rounded-lg bg-white border text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 text-sm transition-colors ${
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2.5">
              {t.builders.label} <span className="text-blue-600">*</span>
            </label>
            ...
            {errors.builders && <p className="mt-1.5 text-red-500 text-xs">{errors.builders}</p>}
          </div>

          {/* Infrastructure radios */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-lg bg-slate-50 border border-slate-200/80">
            {(
              [
                ...
                <div className="flex gap-4">
                  {[
                    ...
                    <label key={val} className="flex items-center gap-1.5 cursor-pointer group">
                      ...
                      />
                      <span className="text-slate-600 text-sm group-hover:text-slate-900 transition-colors">{display}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          <div>
            ...
            <textarea
              ...
              className={`w-full px-4 py-2.5 rounded-lg bg-white border text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 text-sm resize-none transition-colors ${
                errors.problem ? "border-red-400" : "border-slate-300"
              }`}
            />
            {errors.problem && <p className="mt-1.5 text-red-500 text-xs">{errors.problem}</p>}
          </div>

          {/* Deadline */}
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">
              {t.deadline.label} <span className="text-blue-600">*</span>
            </label>
            <select
              value={form.deadline}
              onChange={(e) => setForm({ ...form, deadline: e.target.value })}
              className={`w-full px-4 py-2.5 rounded-lg bg-white border text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 text-sm transition-colors appearance-none ${
                errors.deadline ? "border-red-400" : "border-slate-300"
              }`}
            >
              <option value="" disabled>{t.deadline.placeholder}</option>
              {Object.entries(t.deadline.options).map(([val, label]) => (
                <option key={val} value={val}>{label}</option>
              ))}
            </select>
            {errors.deadline && <p className="mt-1.5 text-red-500 text-xs">{errors.deadline}</p>}
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-lg bg-white border border-slate-300 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 text-sm transition-colors appearance-none"
            >
              <option value="">{t.discovery.placeholder}</option>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold text-sm rounded-lg transition-all duration-150 shadow-sm flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
